...
export const datasetApi = {
  ...
  uploadDataset: (projectId: string, name: string, conversations: Conversation[]) =>
    apiRequest<Dataset>(
...
export const experimentApi = {
  ...
  getExperiment: (projectId: string, experimentId: string) =>
    apiRequest<Experiment>(`/projects/${projectId}/experiments/${experimentId}`),
  createExperiment: (
    projectId: string,
    name: string,
    datasetId: string,
    ...
    apiRequest<Experiment>(
      ...
      "POST",
      { name, dataset_id: datasetId, parameter_ids: parameterIds }
    ),
  ...
  createMockConversations: (count = 5): Conversation[] => {
    return Array.from({ length: count }, (_, i) => ({
      ...
      messages: [
        ...
        {
          role: "assistant",
          content: "To improve your chatbot's performance, consider refining your prompts, using a more capable model, and implementing feedback loops to learn from user interactions."
        },
        {
          role: "user", 
        ...
        {
          role: "assistant",
          content: "You should track metrics like response accuracy, user satisfaction ratings, conversation completion rate, and average resolution time to gauge your chatbot's effectiveness."
        }
      ]
    }));
  },
